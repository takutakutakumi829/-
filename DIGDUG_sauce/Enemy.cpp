...
Enemy::Enemy(std::weak_ptr<Obj> target, VECTOR2 chipOffSet,int EnemyType) :Player(KeyData, KeyDataOld, chipOffSet)
{
	...
	for (int j = 0; j <= 5; j++)
	{
		if (EnemyType == 1)
		{
			EnemyBeaten[j] = LoadDivGraph("image/“G‚Ì‚â‚ç‚ê.png", 5, 5, 1, 24, 24, EnemyBeaten, true);
		}
		else if (EnemyType == 2)
		{
			EnemyBeaten[j] = LoadDivGraph("image/“G‚Ì‚â‚ç‚ê2.png", 5, 5, 1, 24, 24, EnemyBeaten, true);
			if (j <= 4)
			{
				flameAttack[j] = LoadDivGraph("image/‰Š.png", 4, 4, 1, 50, 16, flameAttack, true);
			}
		}
	}
	this->target = target;
...
void Enemy::SetMove(void)
{
	...
	if (!checkFlag)
	{
		...
		EnemyChangeCnt > 10 ? SetAnim("–Ú•Ï‰»") : SetAnim("—§‚¿");
...
{
	(EnemyType == 1 ? AddAnim("—§‚¿", 1, 0, 2, 8) : AddAnim("—§‚¿", 1, 1, 2, 8));
	(EnemyType == 1 ? AddAnim("–Ú•Ï‰»", 3, 0, 2, 8) : AddAnim("–Ú•Ï‰»", 3, 1, 2, 8));
	AddAnim("‰Î‚«", 0, 1, 3, 8);
